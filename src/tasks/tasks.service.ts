...
export class TasksService {
    ...
    async createTask(user: User, dto: createTaskDto): Promise<Task> {
        ...
        const list = await this.listRepository.findOne({
            where: {id: Number(dto.list), project: {user: {id: user.id}}},
        })
    ...
    async getTaskById(user: User, id: number): Promise<Task> {
        const task = await this.taskRepository.findOne({relations: {list: true}, where: {id, list: {project: {user: {id: user.id}}}}});
    ...
    async updateTask(user: User, id: number, dto: createTaskDto) {
    ...
    async moveTask(user: User, id: number, dto: moveTaskDto) {
        ...
        if (task) {
            ...
            if (dto.listId) {
                ...
                if (!list) {
                    ...
                    throw new ForbiddenException("Нет доступа к списку задач")
